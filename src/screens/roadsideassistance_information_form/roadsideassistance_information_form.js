...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, roadsideAssistanceId, numfields;

        const mapItems = MapItems;

        let roadsideassistance = row['roadsideassistance'];

        numfields = Helper.GetAllNumberFields(roadsideassistance);
        if (numfields.length > 0) Helper.UpdateNumberFields(roadsideassistance, numfields);


        // Add Or Update RoadsideAssistance
        rslt = await Support.AddOrUpdateRoadsideAssistance(roadsideassistance, []);
        if (rslt.status) {
            roadsideAssistanceId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the roadsideAssistanceId and navigation entity if it is deos not exist
            let navItem = roadsideassistance.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    ...
                    rslt = await Support.AddOrUpdateRoadsideAssistance(data);
        ...
        global.AlertPopup("success", "RoadsideAssistance is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
