import RoadsideAssistanceJsonConfig from "config/roadsideassistance_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "roadsideassistance", expand: "", exclude: [], func: Support.AddOrUpdateRoadsideAssistance }
]

const FetchRoadsideAssistanceInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(RoadsideAssistanceJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of RoadsideAssistanceJsonConfig[elm]) {
...
const FetchRoadsideAssistanceDetails = async (roadsideassistanceId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (roadsideassistanceId) {
            global.Busy(true);
            let rslt
            // Get RoadsideAssistance Details
            let $expand = [];
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetRoadsideAssistanceSingle(roadsideassistanceId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetRoadsideAssistanceSingle(roadsideassistanceId);
            }
            
            if (rslt.status) {

                const roadsideassistance = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? roadsideassistance : roadsideassistance[source];
...
const Extract = async (roadsideassistanceId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchRoadsideAssistanceInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(roadsideassistanceId)) {
                    await FetchRoadsideAssistanceDetails(roadsideassistanceId, enums).then(({ row, backRow }) => {
